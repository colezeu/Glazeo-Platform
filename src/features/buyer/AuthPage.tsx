...
export default function AuthPage({ auth, onAuthenticated }: AuthPageProps) {
  ...
  if (loginMethod === "otp" && otpStage === "sent") {
    return (
      <div className="min-h-screen bg-[#F8F9FB] flex items-center justify-center px-4">
        <div className="max-w-sm w-full text-center">
          <span className="text-5xl mb-6 block">📧</span>
          <h1 className="text-2xl font-bold text-neutral-900 mb-2">Verifică email-ul</h1>
          <p className="text-neutral-500 mb-6">
            Am trimis un link de autentificare la <strong>{email}</strong>
          </p>
          <p className="text-sm text-neutral-400 mb-8">
            Dă click pe linkul din email pentru a intra în cont.
            Dacă nu vezi email-ul, verifică și folderul Spam.
          </p>
          <button type="button" onClick={resetOtp}
            className="text-[#1A56DB] font-medium hover:underline text-sm">
            ← Înapoi
          </button>
        </div>
      </div>

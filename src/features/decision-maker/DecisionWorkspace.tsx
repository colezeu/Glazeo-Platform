// ══════════════════════════════════════════════
// GLAZEO — Decision Workspace (Phase 4B)
// Orchestrează cele 5 etape DM-001.
// ══════════════════════════════════════════════
import { useState, useMemo } from "react"
import type { DM001Context, DecisionRecord } from "./dm001Data"
import { CONTEXT_QUESTIONS, COMPARISON_CRITERIA } from "./dm001Data"
...
export default function DecisionWorkspace({ onBack }: { onBack: () => void }) {
  const [stage, setStage] = useState<Stage>("intent")
  const [context, setContext] = useState<DM001Context>({
    ceilingType: "unknown",
    doorTraffic: "unknown",
  ...
  const options = useMemo(() => evaluateOptions(context), [context])

  // Decision Record: snapshot creat doar când utilizatorul selectează
  const [record, setRecord] = useState<DecisionRecord | null>(null)

  const recommendedOptions = options.filter((o) => o.recommended)
  const hasContext = context.ceilingType !== "unknown" || context.acousticNeed !== "visual_only"

  // Modificarea contextului după finalizare invalidează Decision Record-ul anterior
  const handleContextChange = (field: keyof DM001Context, value: any) => {
    if (record !== null) {
      setRecord(null)
  ...
  const handleFinalize = () => {
    ...
    const rec = createDecisionRecord(context, options, selectedOptionId)
    setRecord(rec)
    setStage("decision")
  }

  const canProceedToOptions = hasContext
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="font-semibold text-neutral-900 text-sm">
              Compartimentare din sticlă pentru sală de ședințe
            </span>
          </div>
          {/* Stage indicator */}
              ...
              <div key={s} className="flex items-center gap-1">
                <div
                  className={`w-2 h-2 rounded-full ${
                    stage === s ? "bg-[#1A56DB]" : stagesPassed(s) ? "bg-[#059669]" : "bg-neutral-300"
                  }`}
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          <div>
            ...
            <div className="bg-white rounded-xl border border-[#E5E7EB] p-6 mb-6">
              ...
              <p className="text-sm text-neutral-600">
                ...
                dar problema reală este de obicei <strong>continuitatea spațială</strong> — păstrarea
                ...
                <div className="flex flex-wrap gap-2">
                  {q.options.map((opt) => (
                    <button
                      ...
                      onClick={() => handleContextChange(q.id, opt.value)}
            ...
            <div className="flex flex-col gap-4 mb-6">
              {options.map((opt) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-semibold text-neutral-900">{opt.name}</h3>
                        <span
                        ...
                        >
                          {opt.recommended ? "Recomandat" : "Nerecomandat"}
                        </span>
                      </div>
                      <p className="text-xs text-neutral-500 mt-1">{opt.reason}</p>
                    </div>
                  </div>
                  ...
                  {opt.recommended && (
                    <button
                      onClick={() => { handleSelectOption(opt.id); setStage("comparison") }}
                      className="text-xs font-medium text-[#1A56DB] hover:text-[#1E40AF]"
                    >
        ...
        {stage === "comparison" && (
          <div>
                  ...
                  <tr className="border-b border-neutral-200 bg-neutral-50">
                    ...
                    {options.map((opt) => (
                      <th key={opt.id} className={`text-center px-3 py-3 font-medium ${
                        ...
                        {opt.id === "frameless" ? "Frameless" : opt.id === "spider" ? "Spider" : "Aluminiu"}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {COMPARISON_CRITERIA.map((crit) => (
                    <tr key={crit.id} className="border-b border-neutral-100">
                      ...
                      {options.map((opt) => (
                        <td key={opt.id} className={`text-center px-3 py-3 ${
                          opt.id === selectedOptionId ? "bg-[#EFF6FF]/50" : ""
                        }`}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setStage("options")} className="text-sm text-neutral-500 hover:text-neutral-700">
                ← Alege altă opțiune
              </button>
              <button
                onClick={handleFinalize}
                disabled={!canProceedToDecision}
                className={`px-6 py-2 text-sm font-medium rounded-xl transition-colors ${
        ...
        {stage === "decision" && record && (
          <div>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-500">
                  {record.id} · {new Date(record.decidedAt).toLocaleString("ro-RO")}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-[#059669]/30 p-5 mb-4">
              ...
              <p className="text-sm font-semibold text-neutral-900">
                {options.find((o) => o.id === record.selectedOptionId)?.name}
              </p>
            </div>

            {/* Compromisuri acceptate */}
            ...
            <div className="bg-white rounded-xl border border-[#E5E7EB] p-5 mb-4">
              ...
              <ul className="space-y-1">
                {record.lessons.map((l, i) => (
                  <li key={i} className="text-sm text-neutral-700">• {l}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-[#FFF8E1] rounded-xl border border-[#F59E0B]/30 p-5 mb-6">
              ...
              <ul className="space-y-1">
                {record.nextSteps.map((s, i) => (
                  <li key={i} className="text-sm text-[#92400E]">→ {s}</li>
                ))}
              </ul>
            </div>

            <div className="flex gap-3">
              <button onClick={() => { setRecord(null); setSelectedOptionId(null); setStage("context") }}
                className="text-sm text-neutral-500 hover:text-neutral-700">
                ← Reîncepe cu alt context
              </button>
              <button onClick={onBack}
                className="px-4 py-2 text-sm font-medium bg-neutral-100 text-neutral-700 rounded-xl hover:bg-neutral-200">
                Înapoi la proiecte
              </button>
            </div>
          </div>
...
/** Determină dacă o etapă a fost parcursă (pentru indicatorul vizual). */
function stagesPassed(_s: Stage): boolean {
  return false
}
